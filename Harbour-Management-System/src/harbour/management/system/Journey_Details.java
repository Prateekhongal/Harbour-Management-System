...
public class Journey_Details extends JFrame{  //Forth
	...
	String[] items1 =  {"CHENNAI","KOLKATA","VISHAKAPATNAM","GOA","SINGAPORE","MALDIVES"};
	JComboBox comboBox = new JComboBox(items1);
	comboBox.setBounds(150, 100, 150, 27);
	add(comboBox);
		
		
	String[] items2 =  {"CHENNAI","KOLKATA","VISHAKAPATNAM","GOA","SINGAPORE","MALDIVES"};
	...
	Show.addActionListener(new ActionListener(){
		...
		try{
                    ...
                    String str = "select pnr_no,ticket_id,s_code,jny_date,jny_time,src,dst from reservation where src = '"+src+"' and dst = '"+dst+"'";
                    ResultSet rs=c.s.executeQuery(str);
                    
                    if(rs.next()){
                        table.setModel(DbUtils.resultSetToTableModel(rs));
                    }else{
                        JOptionPane.showMessageDialog(null,"No Ships between Source and Destination");
